...
function App() {
  ...
  const handleDeleteItem =async (id) => {
    ...
    const response= await fetch(`http://localhost:9000/v1/api/delete-campaigns/${id}`,{
      method:"DELETE",
      headers: {
...
// Updated Save edited item function with API call
  const handleSaveEdit = async () => {
     try {
      ...
      const response = await fetch( `http://localhost:9000/v1/api/update-campaigns/${campaignId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
...
const handleAddGivenAmount=async()=>{
 const campaignId = editedItem._id || editedItem.id
  const response=await fetch(`http://localhost:9000/v1/api/given-amount/${campaignId}`,{
    method:"POST",
    headers: {
      'Content-Type': 'application/json',
    },
...
const handleDeleteGivenAmount=async(id,given)=>{
  const response=await fetch(`http://localhost:9000/v1/api/delete-given-amount/${id}/${given}`,{
    method:"DELETE",
    headers: {
      'Content-Type': 'application/json',
    }
